...
const signup = async (req, res, next) => {
  ...
  try {
    ...
    console.log(email, uid, passHash);
    let newUser = await user.AddUser(email, uid, passHash, firstname, lastname);
    if (newUser) {
      res.status(200).json({ status: "signup success" });
    } else {
      res.status(401).json({ status: "cannot process signup request." });
    }
  } catch (err) {
    console.log(err);
    res.status(500).json({ status: "cannot process signup request." });
  }
};
...
const ForgotPassword = async (req, res, next) => {
  try {
    ...
    if (!userDetail) {
      ...
      res
        .status(200)
        .json({ status: "user found. Procedding to change password" });
    }
  } catch (error) {
...
const resetPass = async (req, res, next) => {
  ...
  try {
    let passHash = await hash.hashPassword(password, 10);
    let userDetail = await user.changePass(email, passHash);
    if (userDetail) {
      res.status(200).json({ status: "password updated" });
    } else {
      res.status(401).json({ status: "cannot process request" });
    }
  } catch (error) {
    res.status(500).json({ status: "Internal Server error" });
    res.end();
